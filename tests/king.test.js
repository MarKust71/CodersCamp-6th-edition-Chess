...
test("On edge legal moves detection.", ()=>{
    ...
    expect(kingInCorner.findLegalMoves()).toEqual(expect.arrayContaining([{x:1,y:0}, {x:1,y:1}, {x:0,y:1}]))
    expect(kingOnLeftEdge.findLegalMoves()).toEqual(expect.arrayContaining([{x:3,y:0}, {x:5,y:0}, {x:3,y:1}, {x:5,y:1}, {x:4,y:1}]))
    expect(kingOnTopEdge.findLegalMoves()).toEqual(expect.arrayContaining([{x:0,y:3}, {x:0,y:5}, {x:1,y:3}, {x:1,y:4}, {x:1,y:5}]))
    expect(kingOnRightEdge.findLegalMoves()).toEqual(expect.arrayContaining([{x:3,y:7}, {x:5,y:7}, {x:4,y:6}, {x:5,y:6}, {x:3,y:6}]))
    expect(kingOnBottomEdge.findLegalMoves()).toEqual(expect.arrayContaining([{x:7,y:3}, {x:7,y:5}, {x:6,y:3}, {x:6,y:4}, {x:6,y:5}]))
})
...
test("Same side pieces blocks legal moves.", ()=>{
    ...
    expect(kingOne.findLegalMoves()).toEqual(expect.not.arrayContaining([{x:4,y:3}, {x:2,y:3}]));
})
...
test("Allows attacking opponents pieces.", ()=>{
    ...
    expect(kingOne.findLegalMoves()).toEqual(expect.not.arrayContaining([{x:3,y:4}, {x:3,y:2}]));
})
...
test("Don't allow moving on squares and attacking pieces that are backed by enemy.", ()=>{
    ...
    expect(king.findLegalMoves()).toEqual(except.not.arrayContaining([{x:3,y:2},{x:2,y:4},{x:3,y:4},{x:4,y:4}]))
})
